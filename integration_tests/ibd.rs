...
async fn initial_block_download_task(
    ...
    let (mut enforcer_post_setup, thunder_nodes) = setup(bin_paths, res_tx).await?;
    const BMM_BLOCKS: u32 = 16;
    tracing::info!(blocks = %BMM_BLOCKS, "Attempting BMM");
    thunder_nodes.sender.bmm(&mut enforcer_post_setup, BMM_BLOCKS).await?;

    // Check that sender has all blocks, and syncer has 0
    {
        let sender_blocks = thunder_nodes.sender.rpc_client.getblockcount().await?;
        anyhow::ensure!(sender_blocks == BMM_BLOCKS);
        let syncer_blocks = thunder_nodes.syncer.rpc_client.getblockcount().await?;
        anyhow::ensure!(syncer_blocks == 0);
    }
    ...
    tracing::debug!("Checking peer connections");
    check_peer_connection(&thunder_nodes.syncer, thunder_nodes.sender.net_addr().into()).await?;
    tracing::debug!("Syncer has connection to sender");

    const MAX_WAIT_SYNC_MS: u64 = 30_000;
